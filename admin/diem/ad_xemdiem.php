    ...
    <form action="index.php?mod=diem" method="post">
        <div style="text-align:center">
            ...
            <select name="hk" style="width:100px;height: 25px ">
                <?php
                $query="select hocky from hocky1";
            ...
            <select name="nh" style="width:100px;height: 25px ">
                <?php
                $query1="select NAMHOC from namhoc";
                $results1=mysqli_query($conn,$query1);
                while($data1=mysqli_fetch_assoc($results1)){
                    echo "<option value='$data1[NAMHOC]'>$data1[NAMHOC]</option>";
                }
                ?>
            </select>
...
<table width="75%" border="1" cellspacing="0" cellpadding="10" style="margin-left:180px">
    ...
    if(isset($_POST['add'])) {
        foreach ($students as $item) {
            if($_POST['hk']==$item['hocky'] && $_POST['nh']==$item['NAMHOC'] && $_POST['lop']==$item['LH_MA'] && $_POST['mon']==$item['MH_MA']) {
                ?>
                <tr>
                    ...
                    <td><?php echo "<a href='diem/suadiem.php?cma=$item[D_MA]'><button type='button'>Sửa</button></a>"; ?></td>
                    <td><?php echo "<a href='diem/xoadiem.php?cma=$item[D_MA]'  onclick='return XacNhan();'><button type='button'>Xóa</button></a>"; ?></td>
                </tr>
                <?php
            }
        }
